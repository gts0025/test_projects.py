...
class Vector2:
    ...
    def dist(self,other):
        ...
        try:return math.dist((self.x,self.y),(other.x,other.y))
        except: return 0
    ...
    def roundv(self,precission):
        ...
        if precission == 0:
            self.x = int(round(self.x,precission)) 
            self.y = int(round(self.y,precission))
        return self
...
def norm(vector:Vector2):
    ...
    if vector.x*vector.y == 0:
        ...
        h = math.hypot(vector.x,vector.y)
        vector.x /= h
        vector.y /= h
    return vector
...
def add(v1,v2):
    #performs a vector adition using the specified vectors
    return Vector2(v1.x + v2.x,v1.y + v2.y)
    
def sub(vec1,vec2):
    #performs a vector subtraction using the specified vectors
    return Vector2(vec1.x - vec2.x ,vec1.y - vec2.y)
    
def scale(vector,factor = 1):
...
def dist(vec1,vec2):
    ...
    return math.dist((vec1.x,vec1.y),(vec2.x,vec2.y))
...
def rotate(vector, angle):
    #otates the vector by an anglue
    angle_rad = math.radians(angle)
...
def roundv(vector,precission=0):
...
def update_vector(vector,array):
    ...
    vector.y = array[1]
    return vector
def random_vector(x_start = -1,x_end = 1,y_start =-1 ,y_end =1):
